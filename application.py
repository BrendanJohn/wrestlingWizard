...
app.config["SECRET_KEY"] = 'e5fce5faa2e20b203c014f358f73c48f7129084ab1643c9fa6a0f87ff7a546a2'
...
tips = ["Each time a wrestler increases by one level their health points grow by 10",
        ...
        "Winning a match will earn you 25 points"]
...
def users():
    ...
    for user in userRows:
        totalWrestlers = db.execute("SELECT * FROM wrestlers WHERE userid = :userid and deleted = False", userid=user['id'])
...
def history():
    ...
    return render_template("history.html", matchHistory=matchHistory)
...
def match():
    ...
    if request.method == "GET":
        if not myWrestlers:
            return render_template("noWrestlersYet.html")
        else:
